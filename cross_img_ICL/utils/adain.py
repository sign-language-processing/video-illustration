...
def adain(out_feat, value_feat):
    assert (out_feat.size()[:2] == value_feat.size()[:2])
    size = out_feat.size()
    value_mean, value_std = calc_mean_std(value_feat)
    out_mean, out_std = calc_mean_std(out_feat)
    normalized_feat = (out_feat - out_mean.expand(size)) / out_std.expand(size)
    return normalized_feat * value_std.expand(size) + value_mean.expand(size)
