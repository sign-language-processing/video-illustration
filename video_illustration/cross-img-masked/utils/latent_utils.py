...
def load_latents(app_latent_save_path: Path, struct_latent_save_path: Path) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if type(latents_struct) == list:
        latents_app = [l.to(device) for l in latents_app]
        latents_struct = [l.to(device) for l in latents_struct]
    else:
        latents_app = latents_app.to(device)
        latents_struct = latents_struct.to(device)
...
def invert_images(sd_model: AppearanceTransferModel, app_image: Image.Image, struct_image: Image.Image, cfg: RunConfig):
    ...
    zs_app, latents_app = invert(x0=input_app.permute(2, 0, 1).unsqueeze(0).to(device),
                                 pipe=sd_model,
                                 prompt_src=cfg.prompt,
                                 num_diffusion_steps=cfg.num_timesteps,
                                 cfg_scale_src=3.5)
    zs_struct, latents_struct = invert(x0=input_struct.permute(2, 0, 1).unsqueeze(0).to(device),
                                       pipe=sd_model,
                                       prompt_src=cfg.prompt,
                                       num_diffusion_steps=cfg.num_timesteps,
                                       cfg_scale_src=3.5)
