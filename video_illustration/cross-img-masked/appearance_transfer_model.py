...
class AppearanceTransferModel:
    ...
    def set_attention_masks(self, query_mask_path, key_mask_path, value_mask_path):
        if query_mask_path is not None:
            if self.config.inject_32_res:
                self.query_mask_32 = resize_mask(query_mask_path, res=(32,32))
            if self.config.inject_64_res:
                self.query_mask_64 = resize_mask(query_mask_path, res=(64, 64))
        
        if key_mask_path is not None:
            if self.config.inject_32_res:
                self.key_mask_32 = resize_mask(key_mask_path, res=(32,32))
            if self.config.inject_64_res:
                self.key_mask_64 = resize_mask(key_mask_path, res=(64,64))

        if value_mask_path is not None:
            if self.config.inject_32_res:
                self.value_mask_32 = resize_mask(value_mask_path, res=(32,32))            
            if self.config.inject_64_res:
                self.value_mask_64 = resize_mask(value_mask_path, res=(64,64))    
            ...
            def __call__(self,
                ...
                if perform_swap and not is_cross and "up" in self.place_in_unet and model_self.enable_edit:
                    if attention_utils.should_mix_keys_and_values(model_self, hidden_states):
                        ...
                        if hidden_states.shape[1] == 32 ** 2 and model_self.config.inject_32_res:
                            q_mask, v_mask, k_mask = model_self.query_mask_32, model_self.value_mask_32,  model_self.key_mask_32

                        elif  hidden_states.shape[1] == 64 ** 2 and model_self.config.inject_64_res:
                            q_mask, v_mask, k_mask = model_self.query_mask_64, model_self.value_mask_64,  model_self.key_mask_64

                            if q_mask is not None:
                                q_mask = q_mask.view(hidden_states.shape[1])
                                query[OUT_INDEX][q_mask !=0] = query[OUT_INDEX][q_mask !=0] * model_self.config.query_scale

                            if v_mask is not None:
                                v_mask = v_mask.view(hidden_states.shape[1])
                                v_mask = v_mask.expand(hidden_states.shape[0], -1)
                                value[v_mask !=0] = value[v_mask !=0] * model_self.config.value_scale

                            if k_mask is not None:
                                k_mask = k_mask.view(hidden_states.shape[1])
                                k_mask = k_mask.expand(hidden_states.shape[0], -1)
                                key[k_mask !=0] = key[k_mask !=0] * model_self.config.key_scale


                            # v = value.detach().to('cpu')
                            # v1_spatial = v[1].mean(dim=1)
                            # v1_spatial = v1_spatial.view(64,64).numpy()
                            # plt.imshow(v1_spatial, cmap='viridis')
                            # plt.show()
                            
                            
                            # q = query.detach().to('cpu')
                            # q_spatial = q[0].mean(dim=1)
                            
                            # query_spatial = q_spatial.view(64, 64).numpy()
                            # query_spatial = (query_spatial - query_spatial.min()) / (query_spatial.max() - query_spatial.min())
                            # plt.imshow(query_spatial, cmap='viridis')
                            # plt.show()


                        # if model_self.value_mask is not None:
                        #     value = value * model_self.value_mask
                        
                        # if model_self.key_mask is not None:
                        #     key = key * model_self.key_mask
