...
class ICLPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        **kwds):
        ...
        # start_latents = start_code_b.expand(len(prompts), -1, -1, -1)
        ...
        for i, t in enumerate(tqdm(self.scheduler.timesteps, desc="DDIM Sampler")):
            if ref_intermediate_latents is not None:
                ...
                latents_k = ref_intermediate_latents['k'][-1 -i]
                latents_q = ref_intermediate_latents['q'][-1 -i]
                latents_v = ref_intermediate_latents['v'][-1 -i]

                latents_cur, _, _, _ = latents.chunk(4)

                # latents in index 0 is the actual image
                latents = torch.cat([latents_cur, latents_k, latents_q, latents_v])
            ...
            if guidance_scale > 1. and not swap_guidance:
                noise_pred_uncon, noise_pred_con = noise_pred.chunk(2, dim=0)
                noise_pred = noise_pred_uncon + guidance_scale * (noise_pred_con - noise_pred_uncon)
            elif swap_guidance:
                noise_pred_uncon, noise_pred_con = noise_pred.chunk(2, dim=0)
                noise_pred = noise_pred_uncon + guidance_scale * ((T-i) / T) * (noise_pred_con - noise_pred_uncon)
